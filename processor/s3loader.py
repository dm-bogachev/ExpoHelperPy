...
def upload_file(source_file, destination_file):
    ...
    try:
        logger.info(f"Uploading {source_file} to bucket {BUCKET_NAME}...")
        s3.upload_file(source_file, BUCKET_NAME, destination_file)
        logger.info("Upload successful.")

        try:
            s3.head_object(Bucket=BUCKET_NAME, Key=destination_file)
            logger.info(f"✅ Файл '{destination_file}' существует в бакете '{BUCKET_NAME}'.")
        except ClientError as e:
            if e.response['Error']['Code'] == "404":
                logger.warning(f"❌ Файл '{destination_file}' не найден.")
                retval = False
            else:
